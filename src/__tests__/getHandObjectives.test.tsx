...
describe('getHandObjectives', () => {
	...
	it('chi hand no dapai -> prior chi, no dapai', () => {
		const hWanTs = [1, 1, 2, 3, 4, 9].map((i, x) =>
			getSuitedTileMock(Suit.WAN, i, x)
		);
		const hTongTs = [1, 2, 4, 5].map((i, x) => getSuitedTileMock(Suit.TONG, i, x));
		const hSuoTs = [1, 2, 3, 6, 6, 7].map((i, x) =>
			getSuitedTileMock(Suit.SUO, i, x)
		);
	...
	it('melded 3x chi 2S -> prior chi, no prior S, no dapai', () => {
		...
		const ho = getHandObjectives(
			discardCategories,
			[
				`${MeldType.CHI}-7${Suit.TONG}`,
				`${MeldType.CHI}-7${Suit.TONG}`,
				`${MeldType.CHI}-7${Suit.WAN}`
			],
			[Wind.E, Wind.E],
			0
		);
	...
	it('no melds, strong wan, easy scoring pong -> prior pong, dapai & wan', () => {
		const hWanTs = [1, 1, 1, 2, 2, 3, 3, 4, 9].map((i, x) =>
			getSuitedTileMock(Suit.WAN, i, x)
		);
		const hTongTs = [1, 2, 4, 5, 6].map((i, x) => getSuitedTileMock(Suit.TONG, i, x));
		const hDaPai = [DaPai.GREEN, DaPai.GREEN, DaPai.GREEN].map((i, x) =>
			getHBFMock(i, x)
		);
	...
	it('no meld // 3x pong W // 2x pong 1 chi W -> prior P D // P W & D // null W & D', () => {
		const hWanTs = [1, 1, 2, 2, 4, 9, 9].map((i, x) =>
			getSuitedTileMock(Suit.WAN, i, x)
		);
		const hTongTs = [4, 4, 5, 6, 9, 9].map((i, x) =>
			getSuitedTileMock(Suit.TONG, i, x)
		);
		const hDaPai = [DaPai.GREEN, DaPai.GREEN, DaPai.GREEN].map((i, x) =>
			getHBFMock(i, x)
		);
		...
		const ho2 = getHandObjectives(
			discardCategories,
			[
				`${MeldType.KANG}-8${Suit.WAN}`,
				`${MeldType.PONG}-6${Suit.WAN}`,
				`${MeldType.PONG}-7${Suit.WAN}`
			],
			[Wind.E, Wind.E],
			0
		);
